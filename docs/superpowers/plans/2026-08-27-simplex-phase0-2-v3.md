# SimPlex Phase 0–2 Implementation Plan (v3 — implementation-ready)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans. Steps use `- [ ]` checkboxes. This plan is **fully self-contained** — do NOT consult archived plans.
...
pair_scores: pair_id, source_file, well, barcode, sequence_id:0, sequence:1_id, pairing_status,
...
from ._contract import BARCODE_LEN, UMI_LEN
...
def test_disjoint_singletons(): assert resolve({"A"},{"B"})==("mispaired","none",None)
def test_disjoint_one_ambiguous(): assert resolve({"A","B"},{"C"})==("mispaired","none",None)
def test_unique(): assert resolve({"A","B"},{"A"})==("correct","unique","A")
def test_nonunique(): assert resolve({"A","B"},{"A","B"})==("correct","ambiguous",None)
def test_empty(): assert resolve(set(),{"A"})==("unmatchable","none",None)
def test_seq_match():
...
def candidates(seq,locus,key_entry,max_frac=0.06,min_len=50):
    ...
    return hits
def resolve(h,l):
    if not h or not l: return ("unmatchable","none",None)
    inter=h&l
    if not inter: return ("mispaired","none",None)
    if len(inter)==1: return ("correct","unique",next(iter(inter)))
...
def _classify_origin(hset, lset, resident):
    cats=set()
    for h in hset:
        for l in lset:
    ...
    return cats.pop() if len(cats)==1 else "ambiguous"
...
def score(pairplex_output, truth_dir, *, pairplex_metadata=None):
    ...
    for f in files:
        ...
        for r in df.to_dicts():
            ...
            # try BOTH orientations against truth (never trust PairPlex loci for orientation)
            oa=(candidates(s0,"IGH",entry), _lights(s1,entry))   # s0=heavy
            ob=(candidates(s1,"IGH",entry), _lights(s0,entry))   # s1=heavy
            via=[o for o in (oa,ob) if o[0] and o[1]]
            if not via:
                pstat,sres,resolved,origin=("unmatchable","none",None,"unknown")
            elif len(via)==1:
                h,l=via[0]; pstat,sres,resolved=resolve(h,l)
                origin=("resident" if resolved in res_here else "ambient") if (pstat=="correct" and resolved) \
                        else _classify_origin(h&l if pstat=="correct" else h, l if pstat=="correct" else l, res_here) if pstat=="correct" \
                        else _classify_origin(h,l,res_here) if pstat=="mispaired" else "unknown"
            else:
                ra=resolve(*via[0]); rb=resolve(*via[1])
                if ra==rb:
                    h,l=via[0]; pstat,sres,resolved=ra
                    origin=("resident" if resolved in res_here else "ambient") if (pstat=="correct" and resolved) else _classify_origin(h,l,res_here)
                else:
            ...
            rows.append({"pair_id":f"{f.stem}:{r.get('sequence_id:0')}","source_file":str(f),
                "well":well,"barcode":bc,"sequence_id:0":r.get("sequence_id:0"),"sequence:1_id":r.get("sequence_id:1"),
    ...
    pair_scores=pl.DataFrame(rows) if rows else pl.DataFrame(schema={c:pl.Utf8 for c in
        ["pair_id","source_file","barcode","sequence_id:0","sequence:1_id","pairing_status","source_resolution","origin_status","key_status","output_status","resolved_source"]}|{"well":pl.Int64})
...
Identical to plan v2 Task 5 (unchanged): `assign_droplets_and_barcodes(cells,mean,sd,chemistry,barcode_pool_size,seed)` and `assign_wells(cells,wells,seed)`, with the uniformity, pool-reuse-collision, and analytic `Σ_d C(k_d,2)/wells` tests. **Inline the same code and tests here** (see canonical schema; `barcode_pool_size` None → unique per droplet, int → sample droplet barcodes from a pool of that size). Commit `feat(simplex): droplets (barcode_pool_size) + wells + analytic collision test`.

*(Code — append to `simplex/cells.py`):*
```python
...
Same as plan v2 Task 7, plus: returns typed empty `reads` when no molecule survives; sets `amplification_well`/`survived` on **all** molecules (overwriting the placeholders). Tests: all-molecules-kept-with-survival; free keeps barcode+UMI; family shares UMI; index hop; **zero-survival empty reads**.

*(Code — `simplex/routing.py`):*
```python
import numpy as np, polars as pl
from ._rng import rng_for
...
Commit `feat(simplex): routing — survival kept, redistribution, amplification, per-read hop, empty-safe`.
...
def build_truth_components(cells,reads):
    cs=_cc_seq(cells)
    if reads.height==0:
        agg=pl.DataFrame(schema={"final_well":pl.Int64,"barcode":pl.Utf8,"origin_cell_id":pl.Int64,"chain":pl.Int8})
    else:
...
def build_truth_barcodes(cells,truth_cells,components):
    ...
    per=res.group_by([pl.col("final_well").alias("well"),"barcode","origin_cell_id"]).agg([
        ...
        ((pl.col("n_reads_resident").min()>=REF_MIN_READS)&(pl.col("n_umis").min()>=REF_MIN_UMIS)&(pl.col("chain").n_unique()==2)).alias("ref_both")])
    ...
    def dom(loci,by,name):
        ...
        g=(f.group_by([pl.col("final_well").alias("well"),"barcode"])
             .agg([pl.col("source_pair_id").sort_by(by,descending=True).alias("srcs"),pl.col(by).sort(descending=True).alias("vals")]))
        return g.with_columns([pl.col("srcs").list.first().alias(f"dominant_{name}_source_by_{by.replace('n_','')}"),
            (pl.col("vals").list.len()>1) & (pl.col("vals").list.get(0)==pl.col("vals").list.get(1)) .fill_null(False).alias(f"{name}_dominance_is_tied_{by}")]) \
            .select(["well","barcode",f"dominant_{name}_source_by_{by.replace('n_','')}",f"{name}_dominance_is_tied_{by}"])
...
### Task 10: IO writers + run() (guards, full manifest)

**Files:** Create `simplex/io.py`, `simplex/run.py`, `simplex/tests/test_run.py`.

Same as plan v2 Task 10 with: writer described as an **in-memory per-well writer** (not "streaming"); `run()` refuses a non-empty output dir; `run()` wires `chain_status` into `build_truth_cells` and `truth_cells` into `build_truth_barcodes`; **full manifest**.

*(manifest fields):*
```python
import sys, numpy, polars, hashlib, json
try: import pairplex; ppv=getattr(pairplex,"__version__","unknown")
except Exception: ppv="unknown"
manifest={"simplex_version":_SV,"pairplex_version":ppv,"python":sys.version.split()[0],
    ...
    "counts":{"cells":cells.height,"molecules":mols.height,"reads":reads.height,
              "components":comp.height,"keys":tbar.height},
...
```
Tests: outputs+truth exist; manifest has versions/fingerprint/counts; **refuses non-empty dir**; reproducible. Commit `feat(simplex): writers + run() with output-dir guard and full manifest`.
...
```
Implement the other five with the same `family`/`emit` pattern and consistent `chain_status`/`molecules`:
...
HEAVY_A,LIGHT_A=_h[_names[0]],_l[_names[0]]; HEAVY_B,LIGHT_B=_h[_names[1]],_l[_names[1]]
...
def metrics(ppo, truth):
    ...
    correct_keys={(r["well"],r["barcode"]) for r in ps.filter((pl.col("pairing_status")=="correct")&(pl.col("origin_status")=="resident")).to_dicts()}
    refpair=ks.filter(pl.col("reference_pairable_both"))
