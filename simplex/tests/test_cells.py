...
def test_unique_bc_per_droplet(tmp_path):
    c=assign_droplets_and_barcodes(_c(tmp_path),5,0.0,"v2",None,0)
...
def test_pool_reuse_collides(tmp_path):
    c=assign_droplets_and_barcodes(_c(tmp_path),5,0.0,"v2",20,0)
...
def test_analytic_cooccupancy(tmp_path):
    wells=8; c=assign_wells(assign_droplets_and_barcodes(_c(tmp_path,2000),5,0.0,"v2",None,0),wells,0)
