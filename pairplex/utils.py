...
# def process_cell_pair(cell: str, cell_df: pd.DataFrame, metadata: pd.DataFrame, only_pairs: bool = False) -> tuple | None:
...
#         else:
#             pair = False
#             chain = from_polars(cell_df)[0]
#             umi, reads = metadata_df.filter((pl.col("sequence_id") == chain['sequence_id']))[["UMI_count",'reads']].row(0)
#             singlet_dict = {}
#             singlet_dict['index'] = cell
#             for k, v in chain.annotations.items():
#                 singlet_dict[k] = v
#             singlet_dict['umi'] = umi
#             singlet_dict['reads'] = reads
#             singlet_dict['unpairing'] = "Singlet"
#             return (pair, [singlet_dict, ])
...
#             heavy_umi, heavy_reads = metadata_df.filter((pl.col("sequence_id") == heavy['sequence_id']))[["UMI_count",'reads']].row(0)
#             light_umi, light_reads = metadata_df.filter((pl.col("sequence_id") == light['sequence_id']))[["UMI_count",'reads']].row(0)
...
def process_cell_pair(cell: str, cell_df: pd.DataFrame, metadata: pd.DataFrame, only_pairs: bool = False) -> tuple | None:
    ...
    if len(cell_df) == 1:
        ...
        if only_pairs:
            ...
            pair = False
            chain = from_pandas(cell_df)[0]
            umi, reads = metadata[metadata['sequence_id'] == chain['sequence_id']][["UMI_count",'reads']].iloc[0]
            singlet_dict = {}
            singlet_dict['index'] = cell
            for k, v in chain.annotations.items():
                singlet_dict[k] = v
        ...
        chain1 = cell_df[cell_df['locus'] == 'IGH']
        chain2 = cell_df[cell_df['locus'] != 'IGH']

        if len(chain1) == 1 and len(chain2) == 1:
            pair = True
            # Pair the chains
            heavy = from_pandas(chain1)[0]
            light = from_pandas(chain2)[0]
            for k in [k for k in light.annotations.keys() if k.startswith("d")]:
                light.annotations.pop(k)
            
            # Gather the corresponding metadata
            heavy_umi, heavy_reads = metadata[metadata['sequence_id'] == heavy['sequence_id']][["UMI_count",'reads']].iloc[0]
            light_umi, light_reads = metadata[metadata['sequence_id'] == light['sequence_id']][["UMI_count",'reads']].iloc[0]
